...
def stars_list():
    ...
    star_from_db = list(db.mystar.find({}, {'_id':False}).sort('like', -1))
    print(star_from_db)
    # 참고) find({},{'_id':False}), sort()를 활용하면 굿!
    # 2. 성공하면 success 메시지와 함께 stars_list 목록을 클라이언트에 전달합니다.
    return jsonify({'result': 'success','msg':'list 연결되었습니다!', 'stars': star_from_db})
...
def star_like():
    ...
    star_from_db = db.mystar.find_one({'name': name})
    
    if name == star_from_db['name']:
    # 3. star의 like 에 1을 더해준 new_like 변수를 만듭니다.
        new_like = star_from_db['like'] + 1
